...
Please remember to supply a file, a valid x and y coordinate,\
 and a filler character."
...
def autoFill(aMatrix, xCoor, yCoor, rowCount, colCount, fillerChar, fillChar):

    # base case: when an actual boundary or natural boundary is reached
    if (xCoor < 0) or (xCoor >= rowCount) or (yCoor < 0) or (yCoor >= colCount) or (aMatrix[xCoor][yCoor] != fillChar):
        return
    else:
        aMatrix[xCoor][yCoor] = fillerChar
        autoFill(aMatrix, xCoor+1, yCoor, rowCount, colCount, fillerChar, fillChar)
        autoFill(aMatrix, xCoor-1, yCoor, rowCount, colCount, fillerChar, fillChar)
        autoFill(aMatrix, xCoor, yCoor-1, rowCount, colCount, fillerChar, fillChar)
        autoFill(aMatrix, xCoor, yCoor+1, rowCount, colCount, fillerChar, fillChar)

    return aMatrix

# writeToFile() saves the output of autoFill() in a file (matrix1-results.txt)
# input: aMatrix
...
Please try again.\n*"
    ...
    if validCoor(xCoor, yCoor, rowCount, colCount):
        ...
        fillerChar = sys.argv[4]
        fillChar = fileMatrix[xCoor][yCoor]
        newMatrix = autoFill(fileMatrix, xCoor, yCoor, rowCount, colCount, fillerChar, fillChar)
